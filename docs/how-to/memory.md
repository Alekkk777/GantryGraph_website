...
**`MiniVecDbMemory` raises `ValueError: vector must be 384-dimensional`** — your `embed_fn` is returning a vector of the wrong size. Use a model that outputs 384 dimensions (e.g. `text-embedding-3-small` or `all-MiniLM-L6-v2`).
